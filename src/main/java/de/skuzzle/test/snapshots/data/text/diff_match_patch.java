...
/**
 * Class containing the diff, match and patch methods. Also contains the behaviour
 * settings.
 */
class diff_match_patch {
    ...
    /**
     * How far to search for a match (0 = exact location, 1000+ = broad match). A match
     * this many characters away from the expected location will add 1.0 to the score (0.0
     * is a perfect match).
     */
    public int Match_Distance = 1000;
    /**
     * When deleting a large block of text (over ~64 characters), how close do the
     * contents have to be to match the expected contents. (0.0 = perfection, 1.0 = very
     * loose). Note that Match_Threshold controls how closely the end points of a delete
     * need to match.
     */
    ...
    /**
     * Internal class for returning results from diff_linesToChars(). Other less paranoid
     * languages just use a three-element array.
     */
     ...
     * Diff(Operation.EQUAL, " world.")} which means: delete "Hello", add "Goodbye" and
     * keep " world."
     */
     ...
     * This method allows the 'checklines' of diff_main() to be optional. Most of the time
     * checklines is wanted, so default to true.
     *
     ...
     * @param text1 Old string to be diffed.
     * @param text2 New string to be diffed.
     * @param checklines Speedup flag. If false, then don't run a line-level diff first to
    ...
    private LinkedList<Diff> diff_lineMode(String text1, String text2,
        ...
        final LinesToCharsResult a = diff_linesToChars(text1, text2);
        text1 = a.chars1;
        text2 = a.chars2;
        final List<String> linearray = a.lineArray;
        ...
        while (thisDiff != null) {
            ...
            case EQUAL:
                ...
                if (count_delete >= 1 && count_insert >= 1) {
                    ...
                    for (final Diff subDiff : diff_main(text_delete, text_insert, false,
                            deadline)) {
                        pointer.add(subDiff);
                    }
                }
     ...
     * recursively constructed diff. See Myers 1986 paper: An O(ND) Difference Algorithm
     * and Its Variations.
     *
     * @param text1 Old string to be diffed.
     * @param text2 New string to be diffed.
    ...
    /**
     * Split two texts into a list of strings. Reduce the texts to a string of hashes
     * where each Unicode character represents one line.
     *
     * @param text1 First string.
     * @param text2 Second string.
     * @return An object containing the encoded text1, the encoded text2 and the List of
     *         unique strings. The zeroth element of the List of unique strings is
     *         intentionally blank.
     */
    protected LinesToCharsResult diff_linesToChars(String text1, String text2) {
        ...
        final String chars1 = diff_linesToCharsMunge(text1, lineArray, lineHash, 40000);
        final String chars2 = diff_linesToCharsMunge(text2, lineArray, lineHash, 65535);
        return new LinesToCharsResult(chars1, chars2, lineArray);
    }

    /**
     * Split a text into a list of strings. Reduce the texts to a string of hashes where
     * each Unicode character represents one line.
     *
     ...
     * @param text1 First string.
     * @param text2 Second string.
     * @return The number of characters common to the end of the first string and the
     ...
     * text? This speedup can produce non-minimal diffs.
     *
     * @param text1 First string.
     * @param text2 Second string.
     * @return Five element String array, containing the prefix of text1, the suffix of
     *         text1, the prefix of text2, the suffix of text2 and the common middle. Or
     ...
     * @return Five element String array, containing the prefix of longtext, the suffix of
     *         longtext, the prefix of shorttext, the suffix of shorttext and the common
    ...
    public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
        ...
        final Deque<Diff> equalities = new ArrayDeque<Diff>(); // Double-ended queue of
                                                               // qualities.
        ...
        while (thisDiff != null) {
            if (prevDiff.operation == Operation.DELETE &&
                ...
                final int overlap_length1 = this.diff_commonOverlap(deletion, insertion);
                final int overlap_length2 = this.diff_commonOverlap(insertion, deletion);
    ...
    /**
     * Look for single edits surrounded on both sides by equalities which can be shifted
     * sideways to align the edit to a word boundary. e.g: The c<ins>at c</ins>ame. -> The
     * <ins>cat </ins>came.
     *
     * @param diffs LinkedList of Diff objects.
     */
    ...
    /**
     * Given two strings, compute a score representing whether the internal boundary falls
     * on logical boundaries. Scores range from 6 (best) to 0 (worst).
     *
    ...
    /**
     * Reorder and merge like edit sections. Merge equalities. Any edit section can move
     * as long as it doesn't cross an equality.
     *
     * @param diffs LinkedList of Diff objects.
     */
    public void diff_cleanupMerge(LinkedList<Diff> diffs) {
        ...
        /*
         * Second pass: look for single edits surrounded on both sides by equalities which
         * can be shifted sideways to eliminate an equality. e.g: A<ins>BA</ins>C ->
         * <ins>AB</ins>AC
         */
        boolean changes = false;
        // Create a new iterator at the start.
    ...
    /**
     * Compute the Levenshtein distance; the number of inserted, deleted or substituted
     * characters.
     *
     * @param diffs List of Diff objects.
    ...
    /**
     * Crush the diff into an encoded string which describes the operations required to
     * transform text1 into text2. E.g. =3\t-2\t+ing -> Keep 3 chars, delete 2 chars,
     * insert 'ing'. Operations are tab-separated. Inserted text is escaped using %xx
     * notation.
     *
     * @param diffs List of Diff objects.
     * @return Delta text.
     ...
     * required to transform text1 into text2, compute the full diff.
     *
    ...
    /**
     * Locate the best instance of 'pattern' in 'text' near 'loc'. Returns -1 if no match
     * found.
     *
    ...
    protected int match_bitap(String text, String pattern, int loc) {
        ...
        final Map<Character, Integer> s = match_alphabet(pattern);
        ...
        for (int d = 0; d < pattern.length(); d++) {
            ...
            final int finish = Math.min(loc + bin_mid, text.length()) + pattern.length();
            ...
            for (int j = finish; j >= start; j--) {
                ...
                if ((rd[j] & matchmask) != 0) {
                    final double score = match_bitapScore(d, j - 1, loc, pattern);
                    ...
                    if (score <= score_threshold) {
                        ...
                        best_loc = j - 1;
                        if (best_loc > loc) {
                            // When passing loc, don't exceed our current distance from
                            // loc.
    ...
    private double match_bitapScore(int e, int x, int loc, String pattern) {
        final float accuracy = (float) e / pattern.length();
        final int proximity = Math.abs(loc - x);
        if (Match_Distance == 0) {
    ...
    protected Map<Character, Integer> match_alphabet(String pattern) {
        ...
        final char[] char_pattern = pattern.toCharArray();
        for (final char c : char_pattern) {
            s.put(c, 0);
        }
        int i = 0;
        for (final char c : char_pattern) {
    ...
    protected void patch_addContext(Patch patch, String text) {
        ...
        // Add the prefix.
        final String prefix = text.substring(Math.max(0, patch.start2 - padding),
                patch.start2);
        if (prefix.length() != 0) {
            patch.diffs.addFirst(new Diff(Operation.EQUAL, prefix));
        }
        // Add the suffix.
        final String suffix = text.substring(patch.start2 + patch.length1,
                Math.min(text.length(), patch.start2 + patch.length1 + padding));
        if (suffix.length() != 0) {
    ...
    public LinkedList<Patch> patch_make(String text1, String text2) {
        ...
        final LinkedList<Diff> diffs = diff_main(text1, text2, true);
        if (diffs.size() > 2) {
            diff_cleanupSemantic(diffs);
    ...
    /**
     * Compute a list of patches to turn text1 into text2. text1 will be derived from the
     * provided diffs.
     *
    ...
    public LinkedList<Patch> patch_make(LinkedList<Diff> diffs) {
        ...
        final String text1 = diff_text1(diffs);
        return patch_make(text1, diffs);
    }

    /**
     * Compute a list of patches to turn text1 into text2. text2 is ignored, diffs are the
    ...
    /**
     * Compute a list of patches to turn text1 into text2. text2 is not provided, diffs
    ...
    public LinkedList<Patch> patch_make(String text1, LinkedList<Diff> diffs) {
        ...
        final LinkedList<Patch> patches = new LinkedList<Patch>();
        if (diffs.isEmpty()) {
    ...
    public LinkedList<Patch> patch_deepCopy(LinkedList<Patch> patches) {
        final LinkedList<Patch> patchesCopy = new LinkedList<Patch>();
        for (final Patch aPatch : patches) {
            final Patch patchCopy = new Patch();
            for (final Diff aDiff : aPatch.diffs) {
                final Diff diffCopy = new Diff(aDiff.operation, aDiff.text);
     ...
     * @param patches Array of Patch objects
     * @param text Old text.
     * @return Two element Object array, containing the new text and an array of boolean
     *         values.
     */
    public Object[] patch_apply(LinkedList<Patch> patches, String text) {
        ...
        final String nullPadding = patch_addPadding(patches);
        ...
        int delta = 0;
        final boolean[] results = new boolean[patches.size()];
        for (final Patch aPatch : patches) {
            final int expected_loc = aPatch.start2 + delta;
            final String text1 = diff_text1(aPatch.diffs);
            int start_loc;
            int end_loc = -1;
            ...
            if (start_loc == -1) {
                ...
                if (text1.equals(text2)) {
                    ...
                    // indices.
                    final LinkedList<Diff> diffs = diff_main(text1, text2, false);
                    if (text1.length() > this.Match_MaxBits
                        ...
                        for (final Diff aDiff : aPatch.diffs) {
                            if (aDiff.operation != Operation.EQUAL) {
                                final int index2 = diff_xIndex(diffs, index1);
                                if (aDiff.operation == Operation.INSERT) {
                                    // Insertion
    ...
    /**
     * Add some padding on text start and end so that edges can match something. Intended
     * to be called only from within patch_apply.
     *
     * @param patches Array of Patch objects.
     * @return The padding string added to each side.
     */
    public String patch_addPadding(LinkedList<Patch> patches) {
        final short paddingLength = this.Patch_Margin;
        String nullPadding = "";
        ...
        if (diffs.isEmpty() || diffs.getFirst().operation != Operation.EQUAL) {
            ...
            // Grow first equality.
            final Diff firstDiff = diffs.getFirst();
            final int extraLength = paddingLength - firstDiff.text.length();
        ...
        if (diffs.isEmpty() || diffs.getLast().operation != Operation.EQUAL) {
            ...
            // Grow last equality.
            final Diff lastDiff = diffs.getLast();
            final int extraLength = paddingLength - lastDiff.text.length();
    ...
    /**
     * Look through the patches and break up any which are longer than the maximum limit
     * of the match algorithm. Intended to be called only from within patch_apply.
     *
     * @param patches LinkedList of Patch objects.
     */
    public void patch_splitMax(LinkedList<Patch> patches) {
        final short patch_size = Match_MaxBits;
        ...
        Operation diff_type;
        String diff_text;
        final ListIterator<Patch> pointer = patches.listIterator();
    ...
    public List<Patch> patch_fromText(String textline)
            throws IllegalArgumentException {
        final List<Patch> patches = new LinkedList<Patch>();
        if (textline.length() == 0) {
            return patches;
        }
        final List<String> textList = Arrays.asList(textline.split("\n"));
        final LinkedList<String> text = new LinkedList<String>(textList);
        Patch patch;
    ...
    public static class Patch {
        ...
        public String toString() {
            ...
            for (final Diff aDiff : this.diffs) {
                switch (aDiff.operation) {
                case INSERT:
                    text.append('+');
                    break;
    ...
    /**
     * Unescape selected chars for compatability with JavaScript's encodeURI. In speed
     * critical applications this could be dropped since the receiving application will
     * certainly decode these fine. Note that this function is case-sensitive. Thus "%3f"
     * would not be unescaped. But this is ok because it is only called with the output of
