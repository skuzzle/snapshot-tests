...
. Implement your test cases and add one ore more snapshot assertions as shown above.
. When you now execute these tests the first time, serialized snapshots of your test results will be persisted and the tests will fail.
...
snapshots as well as they will otherwise just add clutter to your repository. The framework comes with a sophisticated 
...
==== Orphaned files
When the framework observes a snapshot file which can not be mapped to a test method it is considered an _orphan_. There
are a number of situations in which a snapshot can become an orphan:

* The test containing the snapshot assertion has been removed/renamed.
