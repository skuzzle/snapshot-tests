...
=== Noteable Changes in this Release
...
* The module `snapshot-tests-jackson` has been deprecated in favor of drop-in replacement `snapshot-tests-jackson`. The
deprecated module will be removed with version `2.0.0`.
...
|https://search.maven.org/artifact/{groupId}/snapshot-tests-jaxb-jakarta/{revnumber}/jar[`snapshot-tests-jaxb-jakarta`]
| XML snapshot serialization using `jaxb` with new jakarta dependencies and structured comparison using `xmlunit`.

|https://search.maven.org/artifact/{groupId}/snapshot-tests-jaxb/{revnumber}/jar[`snapshot-tests-jaxb`]
...
----
testImplementation('{groupId}:snapshot-tests-json:{revnumber}') {
...
----
include::{testIncludes}/de/skuzzle/test/snapshots/snippets/SoftAssertions.java[tags=softassertions,indent=0]
----
