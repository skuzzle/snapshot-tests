...
since the previous _major_ version:

* The modules `snapshot-tests-jackson`, `snapshot-tests-jaxb` and `snapshot-tests-jaxb-jakarta` have been removed in
...
* We now use line endings determined from the local git configuration by default. See
<<_dealing_with_line_breaks, Dealing With Line Breaks>> for details about how to configure line ending normalization.

* By default we will now throw an _assertion failure_ if a single snapshot test produces two ore more snapshots with
the same name. As it is sometimes desirable to use the same name twice, this behavior can now be configured per test
via a new setting in `@SnapshotTestOptions`

* `Automatic-Module-Name` of the `-html` module has been fixed to comply with the other data modules.
